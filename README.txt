...
2. !pip install --user gensim scikit-learn matplotlib numpy
...
3. !pip install --user --upgrade gensim numpy==1.24.3 scipy==1.10.1
...
5. !pip install --upgrade gensim==4.2.0
...
# This line uses a Hugging Face model (likely 'distilbert-base-uncased-finetuned-sst-2-english')
...
# Load the summarization model (using a correct model identifier)
...
8. pip install langchain langchain-community cohere
...
9. pip install pydantic wikipedia-api==0.5.4
...
def fetch_institution_details(institution_name: str) -> InstitutionDetails:
    # Initialize Wikipedia API without user_agent parameter
    ...
    # Extract summary (first 4 sentences)
    ...
    # Simple text search for basic information
...
10. !pip install requests PyMuPDF langchain faiss-cpu sentence-transformers numpy python-dotenv
...
class IPCChatbot:
    ...
    def load_pdf(self, pdf_path=None):
        ...
        if pdf_path is None:
            pdf_path = input("Enter path to IPC PDF file: ").strip('"')  # Remove quotes if pasted from Windows
